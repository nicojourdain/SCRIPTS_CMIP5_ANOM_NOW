...
file_out_gridT = 'MMMM_hist_1989-2009_gridT.nc'
file_out_gridU = 'MMMM_hist_1989-2009_gridU.nc'
file_out_gridV = 'MMMM_hist_1989-2009_gridV.nc'
...
DO ivar=1,nvar
    ...
    102 FORMAT('../CLIMO_OCE/',a,'_MMMM_climo_1989_2009.nc')
...
!##2D       lat(:,:) = tmp_lat(:,:)    
  ...
  DO kt=1,mtime  !! calculation for the 12 months
    ...
    do jj=1,mj
      ...
      if ( kt .eq. 1 .and. ji .eq. 10 .and. jj .eq. 300 ) then
        write(*,*) 'kbot, lev(kbot) = ', kbot, lev(kbot)
        write(*,*) '5,    lev(5)    = ', 5, lev(5)
        write(*,*) 'ktop, lev(ktop) = ', ktop, lev(ktop)
      endif
...
call erreur(status,.TRUE.,'create gridT output')
 ...
 status = NF90_PUT_ATT(fidT,NF90_GLOBAL,"history","Created using interp_oce_CMIP5_to_ORCA025_hist.f90")
...
call erreur(status,.TRUE.,'create gridU output')
 ...
 status = NF90_PUT_ATT(fidU,NF90_GLOBAL,"history","Created using interp_oce_CMIP5_to_ORCA025_hist.f90")
...
call erreur(status,.TRUE.,'create')                     
 ...
 status = NF90_PUT_ATT(fidV,NF90_GLOBAL,"history","Created using interp_oce_CMIP5_to_ORCA025_hist.f90")
