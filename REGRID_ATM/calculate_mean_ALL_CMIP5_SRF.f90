...
INTEGER :: fidA, status, dimID_time, dimID_lat, dimID_lon, mtime, mlat, mlon, lat_ID, lon_ID, time_ID, var_rcp_ID, var_his_ID, fidM, kvar, kmod, Nvar, Nmod, var_anom_ID, NN_ID, var_ID, ki, kj, kk, mm
...
REAL*8,ALLOCATABLE,DIMENSION(:) :: lat, lon, time
...
varnam = (/ 'hurs ', 'uas  ', 'vas  ','tas  ', 'ts   ','psl  ', 'ps   ' /) 
...
DO kvar=1,Nvar
  ...
  DO kmod=1,Nmod
    ...
    if ( kmod .eq. 1 ) then
         ...
         ALLOCATE(  lon(mlon)  ) 
         ALLOCATE(  time(mtime)  ) 
         ALLOCATE(  var_rcp(mlon,mlat,mtime)  ) 
         ALLOCATE(  var_his(mlon,mlat,mtime)  )
         ALLOCATE(  mean_anom(mlon,mlat,mtime)  )
         ALLOCATE(  NN(mlon,mlat,mtime)  ) 
    ...
    do kj=1,mlat
      do mm=1,12
        if ( var_his(ki,kj,mm) .ne. misval .and. var_rcp(ki,kj,mm) .ne. misval ) then
          mean_anom(ki,kj,mm) = mean_anom(ki,kj,mm) + var_rcp(ki,kj,mm) - var_his(ki,kj,mm)
          NN(ki,kj,mm) = NN(ki,kj,mm) + 1.0
        endif
      enddo
    enddo
    enddo                                                              
  ...
  do kj=1,mlat
  do mm=1,12
    if ( NN(ki,kj,mm) .lt. 0.5 ) then
      mean_anom(ki,kj,mm) = misval
    else
      mean_anom(ki,kj,mm) = mean_anom(ki,kj,mm) / NN(ki,kj,mm)
    endif
  enddo
  enddo
  enddo
  ...
  status = NF90_DEF_VAR(fidM,TRIM(varnam_anom),NF90_FLOAT,(/dimID_lon,dimID_lat,dimID_time/),var_anom_ID)
  call erreur(status,.TRUE.,"def_var_anomaly_ID")
  status = NF90_DEF_VAR(fidM,"NN",NF90_FLOAT,(/dimID_lon,dimID_lat,dimID_time/),NN_ID)
  call erreur(status,.TRUE.,"def_var_NN_ID")

  SELECT CASE (kvar)
    CASE(1) !! hurs
      status = NF90_PUT_ATT(fidM,var_anom_ID,"standard_name","relative_humidity")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"long_name","Near-Surface Relative Humidity Anomaly = (2080-2100)_rcp85 - (1989-2009)_hist")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"units","%")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
    CASE(2) !! uas
      status = NF90_PUT_ATT(fidM,var_anom_ID,"standard_name","eastward_wind")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"long_name","Eastward Near-Surface Wind Anomaly = (2080-2100)_rcp85 - (1989-2009)_hist")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
    ...
    CASE(3) !! vas
      ...
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"long_name","Northward Near-Surface Wind Anomaly = (2080-2100)_rcp85 - (1989-2009)_hist")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"units","m s-1")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
    CASE(4) !! tas
      ...
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"long_name","Near-Surface Air Temperature Anomaly = (2080-2100)_rcp85 - (1989-2009)_hist")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"units","K")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
    CASE(5) !! ts
      status = NF90_PUT_ATT(fidM,var_anom_ID,"standard_name","surface_temperature")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"long_name","Surface Temperature Anomaly = (2080-2100)_rcp85 - (1989-2009)_hist")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"units","K")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
    CASE(6) !! psl
      status = NF90_PUT_ATT(fidM,var_anom_ID,"standard_name","air_pressure_at_sea_level")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"long_name","Sea Level Pressure Anomaly = (2080-2100)_rcp85 - (1989-2009)_hist")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"units","Pa")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
    CASE(7) !! ps
      status = NF90_PUT_ATT(fidM,var_anom_ID,"standard_name","surface_air_pressure")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"long_name","Surface Air Pressure Anomaly = (2080-2100)_rcp85 - (1989-2009)_hist")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
      status = NF90_PUT_ATT(fidM,var_anom_ID,"units","Pa")
      call erreur(status,.TRUE.,"put_att_var_anom_ID")
    CASE DEFAULT
  ...
  status = NF90_PUT_ATT(fidM,NF90_GLOBAL,"history","Created using calculate_mean_ALL_CMIP5_SRF.f90")
  ...
  DEALLOCATE( lat, lon, time, var_rcp, var_his, mean_anom, NN )
